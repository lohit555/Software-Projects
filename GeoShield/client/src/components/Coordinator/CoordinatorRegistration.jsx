...
const CoordinatorRegistration = ({ onComplete }) => {
  const { registerCoordinator, sendVerificationCode, verifyCode } = useApp();
  ...
  const handleSendCode = async (e) => {
    e.preventDefault();
    try {
      const result = await sendVerificationCode(phone, 'coordinator');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const coordinator = await registerCoordinator({ ...formData, verified: true });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-orange-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full">
          <h2 className="text-3xl font-bold mb-6 text-center">Coordinator Registration</h2>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-orange-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full">
          ...
          {sentCode && (
            <p className="text-sm text-orange-600 text-center mb-4">Demo code: {sentCode}</p>
          )}
