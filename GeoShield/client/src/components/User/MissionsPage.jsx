...
import RequestCard from '../Volunteer/RequestCard';
import ActiveMissionCard from '../Volunteer/ActiveMissionCard';
import RequestFilters from '../Volunteer/RequestFilters';
import { ArrowLeft, Filter } from 'lucide-react';
import { useApp } from '../../context/AppContext';

const MissionsPage = () => {
  const navigate = useNavigate();
  const { requests, volunteers, currentUser } = useApp();
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="bg-green-600 text-white px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold">Volunteer Missions</h1>
        </div>
        <button
          onClick={() => setShowFilters(!showFilters)}
          className="flex items-center gap-2 px-4 py-2 bg-green-700 rounded-lg hover:bg-green-800"
        >
        ...
        <div className="w-3/10 bg-gray-50 overflow-y-auto p-6">
          ...
          <div>
            <h2 className="text-lg font-bold mb-4">
              Available Missions ({filteredRequests.length})
            </h2>
            <div className="space-y-3">
              {filteredRequests.length === 0 ? (
                <p className="text-gray-500 text-sm">No missions match your filters</p>
              ) : (
                filteredRequests
                  .sort((a, b) => {
